...
module.exports = class Engine extends Schmervice.Service {
  async create(payload, userId, value, isUpload = false) {
    try {
      ...
      let response =  value.map(async (element)=>{
        element.created_by = userId;
        element.updated_by = userId;
  ...
  async update(payload, userId, isUpload) {
    try {
      ...
      let fetch = await commonService.fetch(fetchPaylod)
  ...
  async delete(payload,isUpload) {
    try {
      ...
      let deleteRec = await db.whereIn(key, payload.criteria[key]).del();
      if (deleteRec) {
  ...
  async getValueByPath(obj, path) {
    let keys = path.split('.');
    let result = [];
    let filteredArray = keys.filter((value, i) => i !== 0);
    let filteredArrayresult = filteredArray.join('.');
    try {
      let parsedObject = JSON.parse(obj[keys[0]]);
      if (typeof parsedObject === 'object' && parsedObject !== null && !Array.isArray(parsedObject)) {
        let ischeck_array = parsedObject[`${filteredArrayresult}`]
  ...
  async binding(records, condition) {
    ...
    try {
      ...
      let associate_arr = []
      let binddata_arr = []
      let queryOutput = records.records
      for (let i = 0; i < queryOutput.length; i++) {
        ...
        for (let j = 0; j < condition.length; j++) {
          ...
          let primary_key = condition_element.key.primary
          let foreign_key = condition_element.key.foreign
          let model = condition_element.model


          let foreign_key_arr = await this.getValueByPath(query_element, foreign_key)
          ...
          for (let x = 0; x < foreign_key_arr.length; x++) {
            ...
            let query_ = await this.query(model, filter, fields, 0, 10, [])
            let related_data = {}
            let asBind = {}
    ...
    try {
      ...
      if (type == 'create') {
        for (let i = 0; i < file.length; i++) {
          let result = uploadfile(model, file[i], token, type, s3, file_path[i], i)
          if (result.statusCode == 200) {

            let userobj = { model: 'document', user: token }
            let createdoc = await this.create(userobj, token.email, [{
        ...
        for (let j = 0; j < ids.length; j++) {
          ...
          let result = uploadfile(model, file[j], token, type, s3, file_path[j], j)
          if (result.statusCode == 200) {
            if (type === 'update') {
              let updatedoc = await this.update({
              ...
              let deletedoc = await this.delete({
  ...
  async notification(model, payload, account_id) {
    try {
      ...
      let output
      let count = 0
      let module_id = await this.query("model", { name: model }, [], 0, 10, [])
      let get_notification = await this.query("notification", { model_id: module_id.records[0].id.toString(), account_id: account_id }, [], 0, 10, [])
      for (let i = 0; i < get_notification.records.length; i++) {
        const element = get_notification.records[i];
        let template_details = await this.query("notification_template", { id: element.template_id }, [], 0, 10, [])
        let vendor_details = await this.query("vendor", { id: template_details.records[0].vendor_id }, [], 0, 10, [])
        let template_content = await this.query("notification_template_content", {account_id: account_id, account_id: account_id, template_id: template_details.records[0].id }, [], 0, 10, [])
        let credits = await this.query("vendor_credential", { account_id: account_id,vendor_id: vendor_details.records[0].id }, [], 0, 10, [])

        let send_notification_data = build_notification(template_details.records[0], credits, element, payload, model, "email", vendor_details.records[0].vendor_value, template_content.records[0],server)
